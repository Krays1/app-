...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        title: 'Zell0 Desktop - Enhanced Version'
    });

    mainWindow.loadFile('enhanced-index.html');
...
ipcMain.handle('get-audio-devices', async () => {
    try {
        // This will be handled in the renderer process
        if (mainWindow) {
            mainWindow.webContents.send('request-audio-devices');
        }
        return { success: true };
    } catch (error) {
        return { success: false, error: error.message };
...
ipcMain.handle('save-audio-settings', async (event, settings) => {
    try {
        // Save audio settings to a file
        const settingsPath = path.join(app.getPath('userData'), 'audio-settings.json');
...
ipcMain.handle('load-audio-settings', async () => {
    try {
        const settingsPath = path.join(app.getPath('userData'), 'audio-settings.json');
